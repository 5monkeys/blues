...
build_deps = 'cmake git libgtk2.0-dev pkg-config libavcodec-dev ' \
...
def setup():
    ...
    install_from_source(python_executable, user)
...
def setup_build_environment(version, user):
    ...
    with sudo(user):
        with cd(source_base):
            ...
            info('Fetching OpenCV {} source from {}'.format(version, zip_url))
            run('curl -L {zip_url} > {zip_file}'.format(
                zip_url=zip_url,
                zip_file=zip_file))

            run('unzip {}'.format(zip_file))
...
def install_from_source(python_executable, user):
    blueprint = blueprints.get(__name__)

    python_version = python.requested_version()
    ...
    with cd(release_build_path), sudo(user):
        ...
        cmake = run('cmake {opts} ..'.format(opts=opts))
        ...
        info('Installing OpenCV into {}', isinstance())
